...
function getPasswordOptions(userNumCharacter) {
  if (isNaN(userNumCharacter)) {
    alert("Please enter a valid number.");
    return false;
  } else if (parseInt(userNumCharacter) < 8) {
    alert("Password length must be at least 8 characters.");
    return false;
  } else if (parseInt(userNumCharacter) >= 128) {
...
function generatePassword() {
  var userNumCharacter = prompt("How many characters do you want your password to contain?");
  var passwordValid = getPasswordOptions(userNumCharacter);
  if (passwordValid) {
  ...
  if (hasNumbers) {
    ...
    guaranteedChar.push(numericCharacter[Math.floor(Math.random() * numericCharacter.length)]);
  }
  if (hasLowerCase) {
  ...
  var randomChar = [];
  for (var i = 0; i < userNumCharacter; i++) {
